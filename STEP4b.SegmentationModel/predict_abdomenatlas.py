...
if __name__ == '__main__':
    ...
    if args.ids is None:
        ...
        if os.path.exists(args.img_path+'/'+ex+'.npz'):
            ...
            ids = [f+'/ct.nii.gz' for f in pandas.read_csv(args.ids)[col].dropna().astype(str).tolist()]
    ...
    for img_name in tqdm.tqdm(ids):
        ...
        if cls_out is not None:
            ...
            lesion_classes = [c for c in class_list if (('lesion' in c) or ('malign' in c) or ('benign' in c)
                                                         or ('pdac' in c) or ('pnet' in c) or ('cyst' in c)
                                                         or ('background' in c))] #must match update_output_layer_onk / split_cls_outputs_malignancy
