...
class AbdomenAtlasDataset(Dataset):
    def __init__(self, args, mode='train', k_fold=10, k=0, seed=0, all_train=False,
        ...
        self.synth_real_ratio = synth_real_ratio
        self.synth_real_csv = synth_real_csv
        self.uniform_synthetic = uniform_synthetic
        self.uniform_real = uniform_real
        ...
        # ---- Synthetic / Real ratio + uniform-organ sampling setup ----
        self.dynamic_sampling = ((self.synth_real_ratio is not None) or self.uniform_synthetic
                                  or self.uniform_real) and (self.mode == 'train')

        if self.dynamic_sampling:
            if self.synth_real_csv is None or not os.path.exists(self.synth_real_csv):
                raise ValueError(
                    "synth_real_ratio / uniform_synthetic / uniform_real was requested but synth_real_csv is "
                    "missing or invalid. Provide a CSV with columns BDMAP_ID, is_synthetic (0/1)."
                )
            synth_df = pd.read_csv(self.synth_real_csv)
            if "BDMAP_ID" not in synth_df.columns:
                synth_df = synth_df.rename(columns={"BDMAP ID": "BDMAP_ID"})
            id_to_synth = dict(zip(synth_df['BDMAP_ID'], synth_df['is_synthetic'].astype(int)))



            self.case_indices = {'synthetic': [], 'real': []}
            missing_ids = set()
            unresolvable_ids = set()
            for i, p in enumerate(self.img_list):
                bid = self.get_bdmap_id(p)
                flag = id_to_synth.get(bid, None)
                if flag is None:
                    missing_ids.add(bid)
                    continue
                if self.load_augmented and self.save_destination is not None:
                    base_img_name = os.path.basename(self.img_list[i]).replace('.npy', '.npz')
                    base_lab_name = os.path.basename(self.lab_list[i]).replace('.npy', '.npz')
                    img_ok = os.path.exists(os.path.join(self.save_destination, base_img_name))
                    lab_ok = os.path.exists(os.path.join(self.save_destination, base_lab_name))
                    if not (img_ok and lab_ok):
                        unresolvable_ids.add(bid)
                        continue
                self.case_indices['synthetic' if flag == 1 else 'real'].append(i)

            if unresolvable_ids:
                print(f'WARNING: {len(unresolvable_ids)} BDMAP_IDs in synth_real_csv have no valid file in '
                    f'save_destination, excluded from ratio/uniform sampling pools. Example: {list(unresolvable_ids)[:5]}',
                    flush=True, file=sys.stderr)

            if missing_ids:
                print(f'WARNING: {len(missing_ids)} BDMAP_IDs not found in synth_real_csv, '
                      f'excluded from ratio/uniform sampling pools. Example: {list(missing_ids)[:5]}',
                      flush=True, file=sys.stderr)

            
            synthetic_empty = len(self.case_indices['synthetic']) == 0
            real_empty = len(self.case_indices['real']) == 0

            # Only require a pool to be non-empty if it can actually get sampled from.
            need_synthetic = (self.synth_real_ratio is None or self.synth_real_ratio > 0.0) or self.uniform_synthetic
            need_real = (self.synth_real_ratio is None or self.synth_real_ratio < 1.0) or self.uniform_real

            if (need_synthetic and synthetic_empty) or (need_real and real_empty):
                print(f'WARNING: a required synth/real pool is empty after applying synth_real_csv '
                    f'(synthetic n={len(self.case_indices["synthetic"])}, real n={len(self.case_indices["real"])}, '
                    f'synth_real_ratio={self.synth_real_ratio}), disabling ratio/uniform sampling and '
                    f'falling back to default indexing.',
                    flush=True, file=sys.stderr)
                self.dynamic_sampling = False
            else:
                self.organ_indices = {'synthetic': None, 'real': None}
                for pool_name, want_uniform in [('synthetic', self.uniform_synthetic), ('real', self.uniform_real)]:
                    if not want_uniform:
                        continue
                    self.organ_indices[pool_name] = self._build_organ_indices(self.case_indices[pool_name])

                print(f'Dynamic sampling enabled. ratio(synthetic)={self.synth_real_ratio}, '
                    f'synthetic n={len(self.case_indices["synthetic"])}, real n={len(self.case_indices["real"])}, '
                    f'uniform_synthetic={self.uniform_synthetic}, uniform_real={self.uniform_real}',
                    flush=True, file=sys.stderr)

    def _match_organ_column(self, meta_columns, organ):
        """
        Finds the atlas_meta column corresponding to an organ, tolerant of 'gallbladder' vs
        'gall_bladder' naming and 'number of X' / 'X instances' style column names.
        """
        organ_variants = {organ.lower().replace('_', ''), organ.lower().replace('_', ' ')}
        if organ.lower() in ('gallbladder', 'gall_bladder'):
            organ_variants |= {'gallbladder', 'gall bladder', 'gall_bladder'}

        candidates = [c for c in meta_columns if ('number of' in c.lower() or 'instances' in c.lower())]
        for c in candidates:
            c_norm = c.lower().replace('_', ' ')
            for variant in organ_variants:
                if variant.replace('_', ' ') in c_norm:
                    return c
        return None

    def _build_organ_indices(self, indices):
        """
        Buckets a given list of img_list indices by organ presence, using args.atlas_meta
        (per-case 'number of X' / 'X instances' columns) rather than loading full label volumes.
        """
        if self.tumor_classes is None:
            raise ValueError(
                "uniform_synthetic/uniform_real was requested but tumor_classes was not provided to the "
                "dataset, so organ presence columns can't be resolved. Pass tumor_classes=[...] at init."
            )
        if not hasattr(self.args, 'atlas_meta') or self.args.atlas_meta is None:
            raise ValueError(
                "uniform_synthetic/uniform_real was requested but args.atlas_meta is not set. This CSV "
                "(BDMAP ID + per-organ 'number of X' columns) is required to bucket cases by organ."
            )

        meta = pd.read_csv(self.args.atlas_meta)
        meta = meta.set_index('BDMAP ID')

        organ_to_col = {}
        for organ in self.tumor_classes:
            col = self._match_organ_column(meta.columns, organ)
            if col is None:
                print(f'WARNING: could not find an atlas_meta column for organ "{organ}", '
                      f'cases with only this organ will fall into the "unknown" bucket.',
                      flush=True, file=sys.stderr)
            organ_to_col[organ] = col

        organ_buckets = {}
        for i in indices:
            bid = self.get_bdmap_id(self.img_list[i])
            if bid not in meta.index:
                organ_buckets.setdefault('unknown', []).append(i)
                continue
            row = meta.loc[bid]
            organs_present = [organ for organ, col in organ_to_col.items()
                               if col is not None and row[col] > 0]
            if not organs_present:
                organs_present = ['no_tumor']
            for organ in organs_present:
                organ_buckets.setdefault(organ, []).append(i)

        print('Organ buckets for uniform sampling:',
              {k: len(v) for k, v in organ_buckets.items()}, flush=True, file=sys.stderr)
        return organ_buckets

    def _sample_idx(self):
        """Picks the next case according to synth_real_ratio and per-pool uniform flags."""
        if self.synth_real_ratio is not None:
            use_synth = np.random.random() < self.synth_real_ratio
        else:
            use_synth = np.random.random() < 0.5

        pool = 'synthetic' if use_synth else 'real'
        uniform = self.uniform_synthetic if use_synth else self.uniform_real

        if uniform and self.organ_indices[pool]:
            buckets = self.organ_indices[pool]
            organ = random.choice(list(buckets.keys()))
            return random.choice(buckets[organ])
        return random.choice(self.case_indices[pool])
    ...
    def __getitem__(self, idx):
        ...
        if getattr(self, 'dynamic_sampling', False):
        ...
        if self.load_augmented:
            try:
                ...
                for t in range(max_retries):
                    # change index to another one at random, respecting synth/real ratio if enabled
                    if getattr(self, 'dynamic_sampling', False):
                        idx = self._sample_idx()
                    else:
