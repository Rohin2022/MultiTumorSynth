...
class AbdomenAtlasDataset(Dataset):
    def __init__(self, args, mode='train', seed=0, all_train=False,
        ...
        with open(os.path.join(args.data_root, 'list', 'label_names.yaml'), 'r') as f:
            classes_atlas = yaml.load(f, Loader=yaml.SafeLoader)
            #sort--we sorted when saving in nii2npy.py
            classes_atlas = sorted(classes_atlas)
        ...
        if JHH_only:
            ...
            img_name_list+=self.img_name_list_jhh
        #img_name_list = self.img_name_list_jhh
        #print('CAREFUL: LOADING JHH ONLY')
        ...
        self.classes_atlas = classes_atlas
        #print('Classes:')
        ...
        if self.crop_on_tumor:
            lesion_classes = []
            lession_class_names = []
            for i, c in enumerate(classes_jhh):
    ...
    def create_unk_voxels(self, tensor_lab, idx):
        ...
        if sample_name not in self.img_name_list_jhh:
            #print(f'Atlas sample: {sample_name}')
            #atlas is not annotated for pdac, pnet and cyst
            assert len(tensor_lab.shape) == 4, 'Shape of tensor_lab is not 4D, it should be [C, D, H, W]'
            assert tensor_lab.shape[0] == len(self.classes_jhh), 'Number of channels in tensor_lab does not match the number of classes in jhh.'
            unk_voxels = torch.zeros_like(tensor_lab)
            #lesion classes:
            for i, c in enumerate(self.classes_jhh):
                if c.lower() == 'pancreatic_pdac' or c.lower() == 'pancreatic_pnet' or c.lower() == 'pancreatic_cyst':
                    #check if there is any pancreatic lesion in label:
                    pancreatic_lesion_index = self.classes_jhh.index('pancreatic_lesion')
                    if tensor_lab[pancreatic_lesion_index].sum() > 0:
                        #if there is any pancreatic lesion, we do not mark as unknown.
                        unk_voxels[i] = 1.0
            
        else:
            #print(f'JHH sample: {sample_name}')
            assert len(tensor_lab.shape) == 4, 'Shape of tensor_lab is not 4D, it should be [C, D, H, W]'
            unk_voxels = torch.zeros_like(tensor_lab)
            #lesion classes:
            for i, c in enumerate(self.classes_jhh):
        ...
        known_voxels = 1-unk_voxels  # known voxels are the complement of unknown voxels
        label = tensor_lab  # convert to float for comparison
        assert torch.equal((known_voxels*label).float().sum(),label.float().sum()), f'Sample {sample_name}: The unknown region should not cover channels where our label is different from 0---knwon. we got{(known_voxels*label).float().sum()} and {label.float().sum()}'
        return unk_voxels
    ...
    def __getitem__(self, idx):
        ...
        #chek if jhh
        if sample_name in self.img_name_list_jhh:
            num_classes = len(self.classes_jhh)
        else:
            num_classes = len(self.classes_atlas)
        ...
        #if the image is from atlas, add the pdac, pnet and cyst labels to the np_lab, empty
        if sample_name not in self.img_name_list_jhh:
            tmp = []
            for cls in self.classes_jhh:
                tmp.append(np_lab[self.classes_atlas.index(cls)] if cls in self.classes_atlas else np.zeros_like(np_lab[0]))
            np_lab = np.stack(tmp, axis=0)  # stack along the channel dimension
            #print('Added labels, new shape of np_lab:', np_lab.shape, flush=True, file=sys.stderr)
            #print('Classes JHH are:', self.classes_jhh, flush=True, file=sys.stderr)
        ...
        if self.save_augmented:
            ...
            _=self.create_unk_voxels(tensor_lab,idx)
    ...
    def load_augmented_data(self, idx):
        ...
        # 4. Unpack the bits along the same axis.
        if np_lab.shape[0] != len(self.classes_jhh):
            if np_lab.dtype != np.uint8:
                # Possibly the data is already unpacked, or saved incorrectly.
                raise TypeError(f"Expected np_lab to be of type np.uint8, type is {np_lab.dtype} instead.")
            #print('Shape before unpack:', np_lab.shape, flush=True, file=sys.stderr)
            start_unpack = time.time()
            # 4. Unpack the bits along the same axis.
            np_lab = np.unpackbits(np_lab, axis=0)
            ...
            assert np_lab.shape[0] < (len(self.classes_jhh)+10)
            assert np_lab.shape[0] >= (len(self.classes_jhh))
            np_lab = np_lab[:len(self.classes_jhh)]
    ...
    def save_sanity_check(self, img, lab, idx):
        ...
        if self.saved_count < 10:
            ...
            # Save the labels
            for i, cls in enumerate(self.classes_jhh):
