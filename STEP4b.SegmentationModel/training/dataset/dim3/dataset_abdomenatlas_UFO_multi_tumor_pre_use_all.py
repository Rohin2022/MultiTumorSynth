...
def clean_ufo(reports,annotated_tumors,limit_healthy=True,
    ...
    if too_big_mask.any():
        ...
        print(f'Dropping {len(big_ids)} BDMAP_IDs with tumor diameter > 128 mm', flush=True, file=sys.stderr)
    ...
    #if not use_all_data, prohibit crop on organs without known size/number of tumors
    reports['prohibit crop'] = False
    if not use_all_data:
        for i,row in reports.iterrows():
            orgs_of_interest = interest[interest['BDMAP_ID'] == row['BDMAP_ID']]['Standardized Organ'].unique().tolist()
            if row['Standardized Organ'] not in orgs_of_interest:
    ...
    reports['Tumor Size (mm)'] = reports.apply(lambda row: 'u' if (not bool(re.search(r'\d', str(row['Tumor Size (mm)']))) or row['Unknow Tumor Size'] != 'no') else row['Tumor Size (mm)'], axis=1)
...
def generate_sanity_one_per_tumor(img_list, tumors_per_type, slice_only=False, reports=None):
    """
...
class AbdomenAtlasDataset(Dataset):
    def __init__(self, args, mode='train', k_fold=10, k=0, seed=0, all_train=False,
        ...
        self.debug_img_list_UFO = generate_sanity_one_per_tumor(self.UFO_paths, tumors_per_type, 
                                                                slice_only = self.load_slices, reports=self.reports)
    ...
    def __getitem__real(self, idx, name=None, BDMAP_ID=None, debug_dict=False,try_2=False):
        ...
        if self.mode == 'train':
            ...
            if self.load_slices and np_slices is not None:
                ...
                assert tensor_slices.shape==tensor_img.shape, f'Slices shape {tensor_slices.shape} does not match image shape {tensor_img.shape}'
                assert len(tensor_slices.shape) == 5, f'Slices shape {tensor_slices.shape} is not 5D'
            else:
                tensor_slices = None
    ...
    def get_chosen_segment_mask(self, tensor_lab, tumor_segment):
        ...
        for c in self.classes:
            ...
            if (any('segment' in item for item in tumor_segment) or any('liver' in item for item in tumor_segment)) and 'liver_lesion' in c:
                ...
                segment_mask_lesion_ch.append(segment_mask)
                added=True
                if self.debugging:print('Segment added to class:', c, flush=True, file=sys.stderr)
    ...
    def cut_tumor_segment_with_slices(self, tumor_segment_mask, slices_mask, chosen_organ, metadata_df, idx, radius_tolerance=1.5):
        """
    ...
    class segment_options:
        ...
        """
        def __init__(self, segments, slices=None):
            ...
            if self.slices is not None:
                self.priority = []
                self.priority.append(list(set(self.slices['organs_with_all_sizes_and_slices_known'])))#firt piority
                self.priority.append(list(set(self.slices['organs_with_all_sizes_and_some_slices_known'])))#second priority
                self.priority.append(list(set(self.segments['organs_with_only_known_sizes'])))#third piority
                
                #TODO: last priority is organs with unknown sizes
            else:
                #no slice, no priority levels (for now, see TODO)
                self.priority = [list(set(self.segments['organs_with_only_known_sizes']))]
                #TODO: last priority is organs with unknown sizes
    ...
    def crop(self, tensor_img, tensor_lab, idx, d, h, w, slices_mask=None, slices_dict=None):
        if self.tumor_annotated_seg[self.img_list[idx]]:
            ...
            segment_options= self.segment_options(segments=segments, slices=slices_dict)
            ...
            if len(segment_options)==0:
                ...
                return tensor_img, tensor_lab, tumor_dict, 'random', crop_organ, 'no tumor with known sizes', None, None, None
        ...
        for row in tumor_dict.iterrows():
            ...
            if tpe=='segment':
                try:
                    ...
                    if self.debugging:print(f'Location is: {location}; std organ is: {row[1]["Standardized Organ"]}', flush=True, file=sys.stderr)
                    location=row[1]['Standardized Organ']+'_'+location  #e.g. 'liver_segment_1'
            if '/' in location:
            ...
            if in_crop:
                ...
                if self.load_slices:
                    if coerce_to_bool(row[1]['series matches report']) and (not coerce_to_bool(row[1]['impossible slice'])):
