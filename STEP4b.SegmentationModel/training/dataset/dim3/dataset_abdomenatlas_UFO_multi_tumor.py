...
def _normalize_crop_target(selected_tumor):
    """Convert the cropper's `selected_tumor` return value to a human-readable
    string for the sanity-dump `crop_target` field.

    Input shapes (per `self.crop()` in `dataset.crop`):
      - None                           → atlas per-voxel sample (no cropper ran)
      - 'random'                       → random-crop fallback
      - str                            → a single organ / subsegment name
      - list[str]                      → one or more subsegment names
    Returns one of:
      - 'atlas_per_voxel'  (atlas samples)
      - 'random'           (either random-crop deterministic path)
      - 'pancreas'         (organ-level crop)
      - 'pancreas_body' / 'pancreas_head' / 'pancreas_tail' (subsegment crop)
      - multi-name joined by '+' if the cropper targeted multiple subsegments
      - 'unknown'          (anything we can't classify)
    """
    if selected_tumor is None:
        return 'atlas_per_voxel'
    if isinstance(selected_tumor, str):
        return selected_tumor  # 'random' or a single organ name
    if isinstance(selected_tumor, list):
        if len(selected_tumor) == 0:
            return 'unknown'
        return '+'.join(str(x) for x in selected_tumor)
    return 'unknown'
...
# ---------------------------------------------------------------------------
# Pancreas lesion sub-type classification (cyst / pdac / pnet) from the
# per-tumor 'Tumor Type' + 'Type Certainty' columns in the reports dataframe.
# Used by `get_pancreas_lesion_type` (a method of AbdomenAtlasDataset) at the
# end of __getitem__ / load_augmented_data to add `tumor_type` and
# `tumor_type_organ` fields to the returned sample.
# ---------------------------------------------------------------------------

# Keyword priorities, most specific first. See unit-tested reader.
_PANC_LESION_PNET_KEYWORDS = [
    'neuroendocrine', 'neuro-endocrine', 'neuro endocrine',
    'pnet', 'pannet',
    'islet cell',
    'insulinoma', 
    #'gastrinoma', 'carcinoid', 'paraganglioma',
    'endocrine tumor', 'endocrine neoplasm',
]
_PANC_LESION_PDAC_KEYWORDS = [
    'pdac',
...
_PANC_LESION_CYST_KEYWORDS = [
    'ipmn', 'impn', 'ipmt',
    'intraductal papillary mucinous',
    'intrapancreatic mucinous', 'intrapapillary mucinous',
    'mucinous cystic', 'mucinous neoplasm', 'mucinous tumor',
    'cyst',
    'serous cystadenoma', 'serous cyst adenoma', 'serous cystoadenoma',
    'serous neoplasm', 'serous cystic',
    'cystadenoma',
    'fluid collection',
    'multicystic',
    'branch duct',
]
_PANC_LESION_UNKNOWN_SENTINELS = {'', 'nan', 'u', '_', 'none', 'no lesion'}
# Type Certainty values that count as known; anything else -> 'unknown'.
_PANC_LESION_ACCEPTED_CERTAINTY = {'high', 'certain'}


def _classify_pancreas_tumor_type(raw):
    """Map one `Tumor Type` string to one of cyst / pdac / pnet / other / unknown.

    Order matters: PNET checked first (so 'cystic neuroendocrine tumor' -> pnet),
    then CYST (so 'mucinous cystadenocarcinoma' -> cyst), then PDAC (so plain
    'mucinous adenocarcinoma' falls through to pdac).
    """
    if raw is None:
        return 'unknown'
    t = str(raw).strip().lower()
    if t in _PANC_LESION_UNKNOWN_SENTINELS:
        return 'unknown'
    for kw in _PANC_LESION_PNET_KEYWORDS:
        if kw in t:
            return 'pnet'
    for kw in _PANC_LESION_CYST_KEYWORDS:
        if kw in t:
            return 'cyst'
    for kw in _PANC_LESION_PDAC_KEYWORDS:
        if kw in t:
            return 'pdac'
    return 'other'
...
class AbdomenAtlasDataset(Dataset):
    def __init__(self, args, mode='train', k_fold=10, k=0, seed=0, all_train=False,
        ...
        if benign_maligant_only:
            ...
            # Guard: when the reports CSV is UFO-only (e.g. JHH atlas +
            # UCSF/Merlin UFO reports for pancreas training), no atlas IDs
            # appear in self.reports. In that case the malig/benign truth
            # comes from atlas per-voxel sub-type labels, not from the
            # reports CSV — skip this filter to avoid emptying atlas.
            report_ids = set(self.reports['BDMAP_ID'].tolist())
            if any(case in report_ids for case in atlas_name_list):
                with_reports = [case for case in atlas_name_list if case in report_ids]
        ...
        if args.balance_supervision_report_quality and mode == 'train' and (not Atlas_only):
            ...
            reports_high, ids_high, _ = clean_ufo(self.reports,tumor_classes,slice_only=True, use_all_data=False, limit_healthy = (not args.use_all_data), benign_maligant_only=benign_maligant_only,patch_size=patch_size)
            reports_mid,  ids_mid, _ = clean_ufo(self.reports,tumor_classes,slice_only=False, use_all_data=False, limit_healthy = (not args.use_all_data), benign_maligant_only=benign_maligant_only,patch_size=patch_size)
            reports_low,  ids_low, _ = clean_ufo(self.reports,tumor_classes,slice_only=False, use_all_data=True, limit_healthy = (not args.use_all_data), benign_maligant_only=benign_maligant_only,patch_size=patch_size)
            #remove overlaps
            ...
            if mode == 'train' and balance_supervision and (not UFO_only) and (not Atlas_only):
                ...
                if len(atlas_name_list) > len(img_name_list_UFO):
                    if mask_train_proportion == 50:
                        times = len(atlas_name_list) // len(img_name_list_UFO)
                        rem = len(atlas_name_list) % len(img_name_list_UFO)
                    else:
                        target_len = int(round(len(atlas_name_list) * (100 - mask_train_proportion) / mask_train_proportion))
                        target_len = max(target_len, len(img_name_list_UFO)) # ensure we are not reducing the size of the UFO dataset
                        times = target_len // len(img_name_list_UFO)
                        rem = target_len % len(img_name_list_UFO)
                    ...
                    # randomly select / oversample atlas to reach target proportion
                    if mask_train_proportion == 50:
                        times = len(img_name_list_UFO) // len(atlas_name_list)
                        rem = len(img_name_list_UFO) % len(atlas_name_list)
                    else:
                        target_len = int(round(len(img_name_list_UFO) * mask_train_proportion / (100 - mask_train_proportion)))
                        target_len = max(target_len, len(atlas_name_list)) # ensure we are not reducing the size of the atlas dataset
                        times = target_len // len(atlas_name_list)
                        rem = target_len % len(atlas_name_list)
        ...
        def _drop_unloadable_pairs(*idx_dicts):
            """Validate every (img, lab) pair across the given index dicts and
            remove unloadable ones in-place. Returns the set of canonical names
            that were dropped (a name dropped from one root may still be valid
            in another, so the materialization loop only soft-skips when both
            lookups miss for a name in this set)."""
            ...
            def _ok(path):
                try:
                    ...
                    if path.endswith('.npz'):
                        _ = arr.files  # touches zip TOC; surfaces truncation
                    return True
                except Exception:
                    return False

            items = [(d, name, pair) for d in idx_dicts for name, pair in d.items()]
            with ThreadPoolExecutor(max_workers=16) as ex:
                oks = list(ex.map(lambda t: _ok(t[2][0]) and _ok(t[2][1]), items))
            dropped = set()
            for (d, name, _), ok in zip(items, oks):
                if not ok:
                    print(f'[dataset init] dropping unloadable sample: {name}',
                          flush=True, file=sys.stderr)
                    d.pop(name, None)
                    dropped.add(name)
            if dropped:
                print(f'[dataset init] dropped {len(dropped)} unloadable sample(s)',
                      flush=True, file=sys.stderr)
            return dropped

        dropped_names = _drop_unloadable_pairs(atlas_idx, ufo_idx)
            ...
            if UFO_only or this_mask_is_prohibited:
                ...
                if pair is None:
                    if name in dropped_names:
                        continue  # unloadable, validated-out at index time
        ...
        self.non_tumor_crop_chance_unk_size = 0.3
    ...
    def debug(self):
        ...
        for tumor, img_name in self.debug_img_list_Atlas.items():
            ...
            self._safe_debug_dump(dta, tumor+'_'+BDMAP_ID+'_per_voxel_anno.json')
        for tumor, img_name in self.debug_img_list_UFO.items():
            ...
            dta = self.__getitem__(idx=None, name=img_name,debug_dict=True)
            BDMAP_ID = img_name[img_name.find('BDMAP_'):img_name.find('BDMAP_')+len('BDMAP_00001111')]
            self._safe_debug_dump(dta, tumor+'_'+BDMAP_ID+'_report_anno.json')
        self.debugging=False
        print('Debugging finished')
        
    def _ensure_sanity_dir(self, attempts=10):
        """Robustly ensure self.sanity_path exists. Network filesystems
        (Lustre/NFS) can make os.makedirs(exist_ok=True) raise FileExistsError
        from stale metadata even when the directory is actually absent (e.g.
        right after an `rm -r`), so verify with isdir and retry."""
        for _ in range(attempts):
            if os.path.isdir(self.sanity_path):
                return True
            try:
                ...
                pass
            time.sleep(0.05)
        return os.path.isdir(self.sanity_path)

    def _safe_debug_dump(self, dta, filename):
        """Best-effort, non-fatal sanity dump: a filesystem hiccup writing a
        debug artifact must never crash the augmentation job."""
        self._ensure_sanity_dir()
        try:
            with open(os.path.join(self.sanity_path, filename), 'w') as f:
                json.dump(to_jsonable(dta), f)
        except Exception as e:
            print(f'[debug] could not write sanity dump {filename}: {e}', flush=True, file=sys.stderr)
    ...
    def __getitem__(self, idx, name=None, BDMAP_ID=None, debug_dict=False,try_2=False):
        ...
        try:
            ...
            try:
                ...
                if not try_2:
                    print(f'CAREFUL!!! BAD IMAGE FILE {self.img_list[idx]} — drawing a new random case', flush=True, file=sys.stderr)
                    new_idx = int(np.random.randint(len(self.img_list)))
                    return self.__getitem__(new_idx, try_2=True)
            ...
            try:
                ...
                if not try_2:
                    ...
                    if idx==self.__len__()-1:
                        ...
                        return self.__getitem__(0, try_2=True)
                    return self.__getitem__(idx+1, try_2=True)
                raise ValueError('Error loading:', self.lab_list[idx])
        else:
            slices_dict = None
        ...
        if self.mode == 'train':
            ...
            tumor_type, tumor_type_organ = self.get_pancreas_lesion_type(idx)
            ...
            retur = {"image":          tensor_img.clone().float(),
                    ...
                    "tumor_type_organ": tumor_type_organ,
                    # Human-readable label for which crop path was taken
                    # ('pancreas_body' / 'pancreas' / 'random' / ...). Surfaced
                    # in the sanity dump so subseg/organ/random crops can be
                    # distinguished at a glance (see _normalize_crop_target).
                    "crop_target": _normalize_crop_target(selected_tumor),
                    }
            
            if self.UFO_only:
                sanity_assert_no_lesion_mask(retur['label'], self.classes, self.img_list[idx])
                print('asserted no mask',flush=True)
    ...
    def load_augmented_data(self, idx):
        ...
        if self.mode == 'train':
            if self.img_list[idx] not in self.UFO_paths:
                ...
                tumor_malignancy = tumor_slices.clone()
                malignancy_per_voxel = self.get_malignancy_annotated_per_voxel(idx,tensor_lab)
                # Atlas branch: no cropper dispatch → label crop_target as
                # the per-voxel atlas path.
                crop_target = 'atlas_per_voxel'
            else:
                ...
                if torch.tensor(tumor_volumes_in_crop).float().sum() == 0 and chosen_segment_mask.sum()>0:
                    raise ValueError(f'{self.img_list[idx]}tumor_volumes_report should not be all zeros if chosen_segment_mask is not all zeros')
                malignancy_per_voxel = torch.tensor([np.nan]*len(self.classes)).float().to(tensor_img.device)
                # UFO branch: derive the crop-target label from the cached
                # cropper output in the aug-data JSON.
                crop_target = _normalize_crop_target(dta.get('tumor_in_crop'))
    ...
    def assign_labels(self, tensor_lab, idx):
        ...
        # Fetch the per-report pancreas sub-type composition so the
        # unk_voxels mask on each `pancreatic_*` class can be gated
        # per-sub-type below. Without this, every `pancreatic_*`
        # channel gets the same whole-pancreas unk mask — which
        # silences classification_loss and standard_seg on
        # confidently-absent sub-types (e.g. pdac/pnet when the
        # report says "cyst"). Guardrail: any 'unknown'/'other'
        # token keeps every sub-type mask populated — we cannot
        # rule out a sub-type when the report itself is uncertain.
        try:
            _tumor_type_str, _ = self.get_pancreas_lesion_type(idx)
        except Exception:
            _tumor_type_str = ''
        _tt_set = set(_tumor_type_str.split('+')) if _tumor_type_str else set()
        _has_uncertain_type_pan = bool(_tt_set & {'unknown', 'other'})
        _known_subs_pan = _tt_set & {'pdac', 'pnet', 'cyst'}
        ...
        for j,clss in enumerate(self.classes,0):
            ...
            if clss in self.classes_UFO:
                ...
                if not is_lesion:
                    ...
                    if clss=='liver':
                        unk_channels[clss]=j
                        #join all liver segments (only if the UFO npz carries
                        # them — pancreas-focused UFO datasets have 'liver' as
                        # a single channel with no liver_segment_* subsegs).
                        l=torch.zeros(tensor_lab[0].shape).type_as(tensor_lab[0])
                        if self._has_liver_subsegs_UFO:
                        ...
                        if 'liver' in self.classes_UFO:
                            ...
                            if not self._has_liver_subsegs_UFO:
                                raise ValueError('Liver class is missing in UFO, but we do not have liver segments to join either. We cannot assign labels for liver.')
                        label.append(l)
                        ...
                        if 'bladder' in organ:
                            ...
                            if 'adrenal' in organ and clss=='adrenal_lesion':
                                ...
                                if 'liver' in clss:
                                    ...
                                    # `pancreatic_lesion` always gets the full
                                    # pancreas unk mask (the tumor could be
                                    # anywhere in pancreas). Sub-type channels
                                    # (`pancreatic_pdac`/`_pnet`/`_cyst`) only
                                    # keep the mask when the report leaves
                                    # that sub-type possible — either it's
                                    # explicitly listed (`_known_subs_pan`)
                                    # or the report carries an `unknown`/
                                    # `other` token. Otherwise the sub-type
                                    # is confidently absent, so unk stays 0
                                    # and classification_loss + standard_seg
                                    # push that channel's output toward 0.
                                    _sub_pan = next(
                                        (_s for _s in ('pdac', 'pnet', 'cyst')
                                         if _s in clss),
                                        None,
                                    )
                                    if (_sub_pan is None
                                            or _has_uncertain_type_pan
                                            or _sub_pan in _known_subs_pan):
                                        unk_channels_list.append(unk_segments['pancreas'])
                                    else:
                                        unk_channels_list.append(
                                            torch.zeros_like(unk_segments['pancreas']))
                                else:
                                    unk_channels_list.append(unk_segments[organ])#make only the pixels with unknown tumor location be 1, background pixels are 0
                                tumor_present=True
    ...
    def get_pancreas_lesion_type(self, idx):
        """
        Read the pancreas lesion sub-type composition from the per-tumor
        reports metadata for the sample at `idx`. Returns a (tumor_type, organ)
        tuple.

        `tumor_type` is one of:
            - a pure sub-type:   'cyst' | 'pdac' | 'pnet' | 'other'
            - a '+'-joined mix:  'cyst+pdac' | 'cyst+pnet' | 'pdac+pnet'
                                 'cyst+pdac+pnet' | 'cyst+other' |
                                 'pdac+unknown' | 'cyst+other+unknown' | ...
              (alphabetical order, deterministic per call).
            - a fallback:        'unknown'   (every high-certainty row had a
                                 blank/'u'/'nan' Tumor Type, or none existed).
        `organ` is 'pancreas' when there is at least one pancreas row in the
        reports for this BDMAP_ID; 'none' only when no pancreas row exists.

        Note: when `tumor_type` contains 'unknown' or 'other' mixed with a
        known sub-type, the wrapper's gate skips the sample entirely — the
        un-typed tumor could belong to any channel, so confidently using the
        malig/benign ball mask for one specific sub-type would be wrong.

        Similar in spirit to get_malignancy: takes an index, consults
        self.reports_original. Rows whose 'Type Certainty' is not in
        _PANC_LESION_ACCEPTED_CERTAINTY ({'high','certain'}) are NOT
        dropped — instead they contribute an 'unknown' to the composition,
        which the wrapper treats the same as an explicitly-untyped row
        (skip the whole sample). This avoids the failure mode where a
        low-certainty row silently disappears and lets the downstream loss
        supervise e.g. the pdac channel with a malignant ball mask that
        partly belongs to a possibly-different-type tumor.
        Consumers should parse with `set(tumor_type.split('+'))`.
        """
        # Required columns; if any is missing from self.reports_original we
        # gracefully return ('unknown', 'none').
        for col in ('BDMAP_ID', 'Standardized Organ', 'Tumor Type', 'Type Certainty'):
            if col not in self.reports_original.columns:
                raise ValueError(f'Missing required column "{col}" in reports dataframe.')
                #return 'unknown', 'none'

        img_path = self.img_list[idx]
        bid_start = img_path.find('BDMAP_')
        if bid_start < 0:
            return 'unknown', 'none'
        bdmap_id = img_path[bid_start:bid_start + len('BDMAP_00001111')]

        df = self.reports_original
        rows = df[df['BDMAP_ID'] == bdmap_id]
        if len(rows) == 0:
            return 'unknown', 'none'

        panc = rows[rows['Standardized Organ'].astype(str).str.strip().str.lower() == 'pancreas']
        if len(panc) == 0:
            return 'unknown', 'none'

        # Treat low-certainty rows as 'unknown' TYPE (rather than dropping
        # them). A low-certainty pancreas tumor is still a real pancreas
        # tumor — we just don't know its sub-type. Ignoring such rows would
        # let the downstream loss supervise pdac/pnet/cyst using the full
        # malignant/benign ball mask even when PART of that mask belongs to
        # the low-certainty (possibly different sub-type) tumor. Propagating
        # 'unknown' into the composition forces the wrapper's
        # `tt_set & {'other','unknown'}` guard to skip the sample — which is
        # the SAFE behavior: no seg/cls supervision is applied to any of the
        # three sub-type channels when at least one tumor's type is uncertain.
        cert = panc['Type Certainty'].astype(str).str.strip().str.lower()
        cats = set()
        for cert_val, raw in zip(cert, panc['Tumor Type']):
            if cert_val in _PANC_LESION_ACCEPTED_CERTAINTY:
                # Accepted cert: classify by Tumor Type text (may still yield
                # 'unknown' if the text itself is blank/'u'/'nan').
                cats.add(_classify_pancreas_tumor_type(raw))
            else:
                # Low / missing certainty — we can't trust the typed value
                # even if it's present. Treat as unknown.
                cats.add('unknown')

        # No pancreas rows found at all → ('unknown', 'none'). This path is
        # unreachable here because we already returned above when panc is
        # empty, but kept for clarity.
        if not cats:
            return 'unknown', 'none'
        # If the ONLY category we resolved is 'unknown' (no high-cert typed
        # row anywhere on this patient's pancreas), the caller-facing
        # semantics don't change relative to the prior implementation: we
        # mark organ='pancreas' so the wrapper's per-sample loop sees the
        # sample and explicitly routes it through the 'unknown' skip guard.
        # (Previously we returned organ='none' in this case; the net effect
        # on the loss is identical — both skip the sample.)
        if cats == {'unknown'}:
            return 'unknown', 'pancreas'
        # Deterministic '+'-joined composition (alphabetical) so the wrapper
        # can parse per sub-type. Single-category resolves to the bare name,
        # multi-category becomes e.g. 'cyst+pdac', 'cyst+pdac+pnet', or
        # 'pdac+unknown' (which the wrapper will skip — see comment above).
        return '+'.join(sorted(cats)), 'pancreas'
    ...
    def estimate_tumor_volume(self, idx, tumor_segment_crop):
        ...
        for i,size in enumerate(tumors_in_crop):
            if size == 'u':
                ...
                if not sizes:
                    raise ValueError(f'Tumor size {size!r} has all-zero axes — no measurable dimension. Clean this row in the report CSV.')
...
def debug_save_labels(labels: torch.Tensor,
    ...
    # network filesystems (Lustre/NFS) can make makedirs(exist_ok=True) raise
    # FileExistsError when the dir already exists, so swallow that case.
    try:
