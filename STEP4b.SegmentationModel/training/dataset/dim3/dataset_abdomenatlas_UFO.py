...
def _as_bool(s: pd.Series) -> pd.Series:
    """Coerce many common truthy/falsey encodings to boolean."""
    # try numeric first (handles 1/0, 1.0/0.0)
    num = pd.to_numeric(s, errors="coerce")
    is_true_num = num == 1

    # also handle strings like 'true', 'yes', 'y'
    str_true = s.astype(str).str.strip().str.lower().isin({"true","yes","y","1"})

    # direct booleans
    is_true_bool = s == True  # noqa: E712

    return (is_true_num | str_true | is_true_bool).fillna(False)

def clean_ufo(reports, annotated_tumors):
    reports = reports.copy()

    # ----- healthy detection (robust) -----
    if "no lesion" in reports.columns:
        healthy_mask = _as_bool(reports["no lesion"])
    elif "no_lesion" in reports.columns:
        healthy_mask = _as_bool(reports["no_lesion"])
    else:
        raise KeyError("Couldn't find 'no lesion' / 'no_lesion' column in reports")

    interest = {}

    for organ in annotated_tumors:
        df = reports[reports['Standardized Organ'] == organ].copy()
        # NOTE: is the column name exactly 'Unknown Tumor Size'? (your code had 'Unknow')
        if 'Unknown Tumor Size' in df.columns:
            unk_col = 'Unknown Tumor Size'
        else:
            unk_col = 'Unknow Tumor Size' if 'Unknow Tumor Size' in df.columns else None

        # keep known, single numeric sizes
        df = df[df['Tumor Size (mm)'].ne('u') & df['Tumor Size (mm)'].ne('multiple')]
        if unk_col:
            df = df[df[unk_col] == 'no']

        if organ in ['kidney','adrenal_gland','lung','breast','femur']:
            loc = df['Standardized Location'].astype(str).str.lower()
            df = df[loc.str.contains('right') | loc.str.contains('left')]

        interest[organ] = df
        print(f"Number of useful cases for {organ}: {df['BDMAP_ID'].nunique()}")

    interest['healthy'] = reports[healthy_mask]
    ...
    interest_df = pd.concat(interest.values(), ignore_index=True).drop_duplicates()
    print('Total number of useful cases:', interest_df['BDMAP_ID'].nunique())

    ids_of_interest = interest_df['BDMAP_ID'].unique().tolist()
...
class AbdomenAtlasDataset(Dataset):
    def __init__(self, args, mode='train', seed=0, all_train=False,
        ...
        atlas_name_list2 = list(set([f[:len('BDMAP_00000000')] for f in os.listdir(args.data_root) if 'BDMAP' in f and '_gt' not in f]))
        ...
        img_name_list_UFO2 = list(set([f[:len('BDMAP_00000000')] for f in os.listdir(args.UFO_root) if 'BDMAP' in f and '_gt' not in f]))
        ...
        if "BDMAP_ID" not in self.reports.columns:
