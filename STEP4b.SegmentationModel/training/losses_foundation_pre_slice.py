...
def l1_simple_attenuation_classifier(tumor_att_relative, target_att, chosen_segment_mask, class_list):
    ...
    out = get_lesion_channels(out,class_list)
...
def ce_MLP_attenuation_loss(out, target_att, chosen_segment_mask, class_list, labels=None,ct=None, lesion_classes_allowed=None):
    ...
    if len(y)==0:
        return 0*out.sum()  # no penalization, all with unk attenuation
        
    y = torch.tensor(y, dtype=torch.long, device=out.device)
    out = torch.stack(out_clean, dim=0)  # B,3
    assert (len(out.shape) == 2) and (out.shape[0] == y.shape[0]) and (len(y.shape) == 1), f'Output shape is {out.shape}, y shape is {y.shape}'
...
def calculate_loss(model_output, label, unk_voxels, args, matcher,chosen_segment_mask,tumor_volumes_report,tumor_diameters,
    ...
    att_loss=None
    assert attenuation_classifier in ['none', 'simple', 'MLP','large']
    if attenuation_classifier=='simple':
        if isinstance(tumor_attenuation_out,list):#aux loss
            att_loss = 0
            for att in tumor_attenuation_out:
                att_loss += l1_simple_attenuation_classifier(att, tumor_attenuation_label, chosen_segment_mask, classes) + 0*att.sum()
        else:
            att_loss = l1_simple_attenuation_classifier(tumor_attenuation_out, tumor_attenuation_label, chosen_segment_mask, classes) + 0*tumor_attenuation_out.sum()
    if attenuation_classifier in ['MLP','large']:
        if isinstance(tumor_attenuation_out,list):#aux loss
            att_loss = 0
            for att in tumor_attenuation_out:
                att_loss += ce_MLP_attenuation_loss(att, tumor_attenuation_label, chosen_segment_mask, classes, labels=label, ct=input_tensor, lesion_classes_allowed=lesion_classes) + 0*att.sum()
        else:
            att_loss = ce_MLP_attenuation_loss(tumor_attenuation_out, tumor_attenuation_label, chosen_segment_mask, classes, labels=label, ct=input_tensor, lesion_classes_allowed=lesion_classes) + 0*tumor_attenuation_out.sum()
