...
class Codebook(nn.Module):
    ...
    def _init_embeddings(self, z):
        # Don't set _need_init = False until initialization succeeds
        ...
        self._need_init = False  # ← only mark done after success
    ...
    def forward(self, z):
        ...
        # EMA codebook update
        if self.training:
            # 1. Safely sum in float32 to prevent exceeding 65,504
            n_total = encode_onehot.float().sum(dim=0) 
            
            # 2. Safely matmul in float32 to prevent overflow
            encode_sum = flat_inputs.float().t() @ encode_onehot.float() 
        ...
        # Cast encode_onehot to float32 before calculating mean and log
