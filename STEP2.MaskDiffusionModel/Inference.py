...
def generate_samples(train_data, step, diffusion, cond_scale=2.0):
    ...
    tumor_mask_dims = (batch_size, 1, 48, 48, 48)
    ...
    img_out,_ =  ddim_sampler.sample(
        200, 8, (1, 48, 48, 48), conditioning=cond, tabular_cond=tabular_cond)
        ...
        for b_idx in range(raw_np.shape[0]):
            ...
            for key in metrics.keys():
                ...
                if(normalized_stats[key]["log_scaled"]):
                    target_real_val = np.expm1(target_real_val)
                print(f"  {key}:")
                ...
                output_metrics.append({"cond_scale": cond_scale, "column": key,
                                      "desired_val": target_real_val, "actual_val": metrics[key]})
...
def reconstruct(cfg: DictConfig):
    ...
    cfg.dataset.datafile="RadiomicsMaskMetricsBadDropped_Test.csv"
    ...
    for train_data in tqdm(loader_iter):
        ...
        for scale in cond_scales:
            output_metrics = generate_samples(
                train_data, step+1, diffusion, cond_scale=scale)
    ...
    metrics_df.to_csv("inference_statistics.csv", index=False)
