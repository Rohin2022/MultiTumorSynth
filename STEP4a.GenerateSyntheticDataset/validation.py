...
def _get_model(args):
    ...
    if args.checkpoint:
        ...
        model_dict = torch.load(os.path.join(args.log_dir, 'model.pt'),weights_only=False)
...
def main():
    ...
    with torch.no_grad():
        for idx, val_data in enumerate(val_loader):
            ...
            name = val_data["label"].meta["filename_or_obj"][0].split("/")[-1].split(".")[0]
            original_affine = val_data["label"].meta["affine"][0].numpy()
            pixdim = val_data["label"].meta["pixdim"].cpu().numpy()
