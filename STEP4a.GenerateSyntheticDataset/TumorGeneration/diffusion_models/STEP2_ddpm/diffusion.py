...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def sample(self, heatmap, organ_mask, tabular_cond=None, cond_scale=1., batch_size=None):
        device = next(self.denoise_fn.parameters()).device

        # Match forward(): permute to (B, C, D, H, W), then concat organ_mask + heatmap
        organ_mask = organ_mask.permute(0, 1, -1, -3, -2).to(device)
        heatmap = heatmap.permute(0, 1, -1, -3, -2).to(device)
...
def compute_voxel_volume(mask, spacing):
    """
    Computes:
        - original_shape_VoxelVolume (mm^3)
        - num_components

    Equivalent to PyRadiomics VoxelVolume for the entire mask.
    """
    ...
    volume_mm3 = np.sum(bin_mask) * voxel_volume_mm3

    return {
        "original_shape_VoxelVolume": float(volume_mm3),
...
class Trainer(object):
    ...
    def load(self, milestone=-1, map_location=None, **kwargs):
        # Load the latest numeric checkpoint if milestone == -1
        if milestone == -1:
            checkpoints = [
                int(p.stem)
            ...
            assert checkpoints, (
                "Need to have at least one numeric checkpoint to load "
                "from the latest checkpoint (milestone == -1)."
            )

            milestone = max(checkpoints)

        checkpoint_path = Path(self.results_folder) / f"{milestone}.pt"
                ...
                with torch.no_grad():
                    ...
                    with open("dataset_norm_stats.json", "r") as f:
                        ...
                        for b_idx in range(min(3, masks_np.shape[0])):
                            ...
                            real_raw = tumor_mask[b_idx, 0, :, :, :].cpu().numpy()
                            ...
                            if pred_3d.sum() > 0:
                                ...
                                for key in metrics.keys():
                                    ...
                                    print("================")
                                    if (key == "original_shape_VoxelVolume"):
                                        volume_delta = abs(
                                ...
                                nib.save(
                                    nib.Nifti1Image(real_raw, affine=affine),
                                    str(debug_folder /
                                        f"step_{self.step}_sample_{b_idx}_REAL_RAW_{volume}.nii.gz")
                                )
